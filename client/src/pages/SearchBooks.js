...
const SearchBooks = () => {
  ...
  const handleFormSubmit = async (event) => {  // create method to search for books and set state on form submit
  ...
  const handleSaveBook = async (bookId) => {   
    const bookBeingSaved = searchedBooks.find((book) => book.bookId === bookId);     
    ...
    try {
      await saveBook({   
        variables: { bookData: { ...bookBeingSaved } },
        context: {
          headers: {
      ...
      setSavedBookIds([...savedBookIds, bookBeingSaved.bookId]);
    } catch (err) {
      console.error(err);
    }
  };
